...
class AdaptiveThresholdManager:
    def __init__(self, initial_threshold, base_alpha=0.05, max_alpha=0.2, min_alpha=0.01, window_size=50, use_adaptive=True, margin_multiplier=1.5):
    ...
    def _calculate_workload_alpha(self):
        ...
        attack_ratio = sum(self.recent_labels) / len(self.recent_labels) if self.recent_labels else 0.0
    ...
    def update(self, mse, is_attack=0, timestamp=None):
        ...
        if is_attack == 0:
            ...
            new_threshold = (1 - alpha) * old_threshold + alpha * (mse * self.margin_multiplier) # simple margin
            
            self.current_threshold = new_threshold
            
            logger.info(f"Threshold Update: {old_threshold:.6f} -> {self.current_threshold:.6f} | alpha: {alpha:.4f}")
