...
agg_df = df.groupby(["algo", "w_time", "w_mse", "w_var", "margin", "alpha"]).mean().reset_index()
...
for algo in df["algo"].unique():
    ...
    print(pareto_pts[["margin", "alpha", "w_time", "w_mse", "w_var", "fpr", "recall", "precision"]].to_string(index=False))

    # Plot ROC-style (FPR vs Recall)
    plt.figure(figsize=(10, 6))
    sns.scatterplot(data=algo_df, x="fpr", y="recall", hue="margin", palette="viridis", alpha=0.6)
    
    plt.plot(pareto_pts["fpr"], pareto_pts["recall"], color='red', marker='o', linestyle='-', linewidth=2, label='Pareto Frontier')
    ...
    plt.figure(figsize=(10, 6))
    sns.scatterplot(data=algo_df, x="recall", y="precision", hue="margin", palette="magma", alpha=0.6)
    
    # Sort pareto pts for PR curve (sort by recall)
    pareto_pr = algo_df[algo_df["is_pareto"]].sort_values("recall")
    plt.plot(pareto_pr["recall"], pareto_pr["precision"], color='red', marker='o', linestyle='-', linewidth=2, label='Pareto Frontier')
