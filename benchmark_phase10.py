...
def run_benchmark():
    ...
    if not hw_avail:
        print(
            "[WARNING] Hardware telemetry is unavailable on this platform (likely macOS or missing Linux perf privileges)."
        )
        print(
            "The benchmark will proceed to validate the graceful fallback, but the HW-only and Hybrid comparisons will mirror the Software-only results."
        )
