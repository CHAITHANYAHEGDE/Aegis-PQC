...
def main():
    parser = argparse.ArgumentParser(
        description="Phase 6 Comparative Benchmarking Harness"
    )
    ...
    test_df = generate_test_sequence(
        args.algo, num_normal=500, num_attack=100, seed=args.seed
    )
    ...
    if n_diff == 0:
        ...
        logger.info(
            f"Wilcoxon signed-rank test (Correctness): p-value = {p_value:.4e}, effect_size (rank-biserial) = {effect_size:.4f}"
        )
