...
ALGORITHMS = [
    ...
    "ML-DSA-44", "Falcon-512", "sphincs+-sha2-128f-simple"
]

SEEDS = [42, 123, 1001, 2024, 8888, 777, 999, 1111, 2222, 3333,
         4444, 5555, 6666, 7777, 9876, 5432, 1999, 2000, 2010, 2026]
...
def run_experiment(algo, seed):
    ...
    metrics_a = calculate_metrics(y_true, pipeline_a_preds, pipeline_a_scores, pipeline_a_latency)
    metrics_b = calculate_metrics(y_true, pipeline_b_preds, pipeline_b_scores, pipeline_b_latency)
...
def plot_results(metrics_df):
    ...
    melt_fpr = metrics_df.melt(id_vars=["algo", "seed"], value_vars=["base_fpr", "adapt_fpr"], var_name="Pipeline", value_name="FPR")
    melt_fpr["Pipeline"] = melt_fpr["Pipeline"].replace({"base_fpr": "Baseline", "adapt_fpr": "Adaptive (Phase 6)"})
    ...
    melt_recall = metrics_df.melt(id_vars=["algo", "seed"], value_vars=["base_recall", "adapt_recall"], var_name="Pipeline", value_name="Recall")
    melt_recall["Pipeline"] = melt_recall["Pipeline"].replace({"base_recall": "Baseline", "adapt_recall": "Adaptive (Phase 6)"})
    ...
    melt_lat = metrics_df.melt(id_vars=["algo", "seed"], value_vars=["base_mean_latency_ms", "adapt_mean_latency_ms"], var_name="Pipeline", value_name="Latency (ms)")
    melt_lat["Pipeline"] = melt_lat["Pipeline"].replace({"base_mean_latency_ms": "Baseline", "adapt_mean_latency_ms": "Adaptive (Phase 6)"})
    
    sns.violinplot(data=melt_lat, x="algo", y="Latency (ms)", hue="Pipeline", split=True)
...
def main():
    ...
    for algo in ALGORITHMS:
        ...
        for seed in SEEDS:
            ...
            # Checkpoint Raw
            raw_df.to_csv(raw_results_file, mode='a', header=not os.path.exists(raw_results_file), index=False)
            
            # Checkpoint Metrics
            pd.DataFrame([metrics_row]).to_csv(metrics_file, mode='a', header=not os.path.exists(metrics_file), index=False)
    ...
    logger.info("\n" + summary[["base_fpr", "adapt_fpr", "base_recall", "adapt_recall", "wilcoxon_p", "effect_size"]].to_string())
