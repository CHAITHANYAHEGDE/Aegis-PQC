...
# Parameter Grid
GRID_WEIGHTS = [(0.33, 0.33, 0.33), (0.6, 0.2, 0.2), (0.2, 0.6, 0.2), (0.2, 0.2, 0.6), (0.8, 0.1, 0.1)]
...
def run_experiment(algo, seed):
    ...
    param_combinations = list(itertools.product(GRID_WEIGHTS, GRID_MARGINS, GRID_ALPHAS))
    ...
    for weights, margin, base_alpha in param_combinations:
        ...
        for i in range(len(y_seq)):
            use_adaptive = selector.update(preds[-1] if i > 0 else 0, y_seq[i-1] if i > 0 else 0)
            adapt.use_adaptive = use_adaptive
            
            fused_score, _ = fusion.compute_fused_score(cached_timing[i], cached_mean_mse[i], cached_var_mse[i])
