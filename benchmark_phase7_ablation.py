...
def custom_engineer_features(df, scaler=None, drop_indices=None):
    ...
    df["rolling_latency_mean"] = df["execution_time_us"].rolling(window=10, min_periods=1).mean()
    df["rolling_latency_std"] = df["execution_time_us"].rolling(window=10, min_periods=1).std().fillna(0)
    ...
    df["telemetry_entropy"] = df["execution_time_us"].rolling(window=20, min_periods=2).apply(compute_entropy, raw=True).fillna(0)
...
def run_experiment(algo, seed):
    ...
    ae_full = PyTorchAutoencoderModel(input_dim=f_train_full["X_seq"].shape[1], epochs=30)
    ae_full.fit(f_train_full["X_seq"])
    f_test_full = custom_engineer_features(test_df, scaler=f_train_full["scaler"], drop_indices=None)

    # 2. No Context/Memory
    f_train_nocmem = custom_engineer_features(train_df, drop_indices=MEMORY_FEAT_IDXS)
    ae_nocmem = PyTorchAutoencoderModel(input_dim=f_train_nocmem["X_seq"].shape[1], epochs=30)
    ae_nocmem.fit(f_train_nocmem["X_seq"])
    f_test_nocmem = custom_engineer_features(test_df, scaler=f_train_nocmem["scaler"], drop_indices=MEMORY_FEAT_IDXS)

    # 3. No Timing
    f_train_notime = custom_engineer_features(train_df, drop_indices=TIMING_FEAT_IDXS)
    ae_notime = PyTorchAutoencoderModel(input_dim=f_train_notime["X_seq"].shape[1], epochs=30)
    ae_notime.fit(f_train_notime["X_seq"])
    f_test_notime = custom_engineer_features(test_df, scaler=f_train_notime["scaler"], drop_indices=TIMING_FEAT_IDXS)

    # Run Pipelines
    y_true = f_test_full["y_seq"]
    pipelines = {
        "full": {"preds": [], "ae": ae_full, "X": f_test_full["X_seq"], "fusion": TelemetryFusion(), "selector": RuleBasedSelector(), "adapt": AdaptiveThresholdManager(initial_threshold=ae_full.threshold), "use_adapt": True},
        "nocmem": {"preds": [], "ae": ae_nocmem, "X": f_test_nocmem["X_seq"], "fusion": TelemetryFusion(), "selector": RuleBasedSelector(), "adapt": AdaptiveThresholdManager(initial_threshold=ae_nocmem.threshold), "use_adapt": True},
        "notime": {"preds": [], "ae": ae_notime, "X": f_test_notime["X_seq"], "fusion": TelemetryFusion(), "selector": RuleBasedSelector(), "adapt": AdaptiveThresholdManager(initial_threshold=ae_notime.threshold), "use_adapt": True},
        "noadapt": {"preds": [], "ae": ae_full, "X": f_test_full["X_seq"], "fusion": TelemetryFusion(), "selector": RuleBasedSelector(), "adapt": AdaptiveThresholdManager(initial_threshold=0.5), "use_adapt": False},
    }

    # Disable sub-loggers
    for logger_name in ["aegis.phase6.fusion", "aegis.phase6.adaptive", "aegis.phase6.selector"]:
    ...
    for i in range(len(y_true)):
        ...
        for p_name, p in pipelines.items():
            ...
            if p["use_adapt"]:
                use_adaptive = p["selector"].update(p["preds"][-1] if i > 0 else 0, y_true[i-1] if i > 0 else 0)
                p["adapt"].use_adaptive = use_adaptive
            else:
                p["adapt"].use_adaptive = False
                
            fused_score, _ = p["fusion"].compute_fused_score(raw_timing, mean_mse[0], var_mse[0])
...
def main():
    ...
    for algo in ALGORITHMS:
        ...
        for seed in SEEDS:
            ...
            pd.DataFrame([res]).to_csv(metrics_file, mode='a', header=not os.path.exists(metrics_file), index=False)
