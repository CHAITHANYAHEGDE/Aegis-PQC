...
class VerificationEvaluator:
    ...
    def compute_metrics(y_true, y_pred, y_prob):
        ...
        return {
            ...
            'MCC': mcc, 'Balanced_Accuracy': bal_acc, 'FPR': fpr
        }
...
def run_verification_benchmark():
    ...
    models_to_test = {
        'GRU': lambda input_dim: GRUClassifier(input_dim=input_dim, hidden_dim=32, num_layers=1, epochs=15),
        'LSTM': lambda input_dim: LSTMClassifier(input_dim=input_dim, hidden_dim=32, num_layers=1, epochs=15),
    ...
    for algo in algorithms:
        ...
        unseen_raw, unseen_lbl = generate_overlapping_telemetry(num_samples=1000, mixed=True)
        unseen_raw_scaled = scaler.transform(unseen_raw)
        
        for seq_len in seq_lengths:
            X_train, y_train = create_sequences_no_leakage(train_raw_scaled, train_lbl, seq_len)
            X_test, y_test = create_sequences_no_leakage(test_raw_scaled, test_lbl, seq_len)
            X_unseen, y_unseen = create_sequences_no_leakage(unseen_raw_scaled, unseen_lbl, seq_len)
            ...
            for model_name, model_fn in models_to_test.items():
                ...
                metrics = VerificationEvaluator.compute_metrics(y_test, preds_test, probs_test)
                ...
                if model_name in ['GRU', 'LSTM']:
                    ...
                    shuffled_metrics = VerificationEvaluator.compute_metrics(shuffled_y, preds_test, probs_test)
                    if shuffled_metrics['ROC_AUC'] > 0.65:
                        logger.error(f"Sanity Check Failed for {model_name}: Random ROC AUC = {shuffled_metrics['ROC_AUC']}")
                ...
                unseen_metrics = VerificationEvaluator.compute_metrics(y_unseen, preds_unseen, probs_unseen)
                
                # Point 8: Latency Profiling
                latency_stats = profile_latency(model, X_test, runs=100)
                
                res = {
                    'Algorithm': algo, 'Model': model_name, 'Seq_Len': seq_len,
                    'Test_Type': 'Standard'
                }
                res.update(metrics)
