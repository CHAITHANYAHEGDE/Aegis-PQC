...
def get_latest_experiment_dir(base_dir="experiments"):
    if not os.path.exists(base_dir):
        return None
    experiments = [os.path.join(base_dir, d) for d in os.listdir(base_dir) if os.path.isdir(os.path.join(base_dir, d))]
    
    # Filter to only experiments that completed and saved metrics.json
    valid_experiments = [d for d in experiments if os.path.exists(os.path.join(d, "metrics.json"))]
...
def get_best_model_from_latest_experiment(base_dir="experiments"):
    ...
    best_res = max(metrics, key=lambda x: (x.get("f1", 0), x.get("mcc", 0), x.get("roc_auc", 0)))
