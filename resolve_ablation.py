...
pi_ablated = permutation_importance(
    rf_ablated, X_test_ablated, y_test, n_repeats=5, random_state=42
)
pi_ablated_df = pd.DataFrame(
    {
        "feature": [f for f in features if f != "execution_time_us"],
        "importance": pi_ablated.importances_mean,
    }
)
